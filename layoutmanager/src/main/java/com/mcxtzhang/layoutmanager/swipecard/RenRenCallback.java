...
public class RenRenCallback extends ItemTouchHelper.SimpleCallback {
    ...
    public RenRenCallback(RecyclerView rv, RecyclerView.Adapter adapter, List datas) {
        this(0,
                ...
                rv, adapter, datas);
        Log.d("RenRenCallback", "datas.size():" + datas.size());
    }
    ...
    public void onSwiped(RecyclerView.ViewHolder viewHolder, int direction) {
        Log.d("test", "onSwiped() called with: viewHolder = [" + viewHolder + "], direction = [" + direction + "]");
        //rollBack(viewHolder);
        //★实现循环的要点
...
//        mDatas.add(0, remove);
    ...
    public interface SwipCallBack {
        void swip(boolean isLeftSwip);
    }

    public void setSwipCallBack(SwipCallBack callBack) {
        mCallBack = callBack;
    }

    protected SwipCallBack mCallBack;
